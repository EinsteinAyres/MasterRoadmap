
# 🚀 Roadmap Backend Python

## 📃 0. Documentações Python

[https://docs.python.org/3/](https://docs.python.org/3/)

[https://realpython.com/](https://realpython.com/)

[https://www.w3schools.com/python/](https://www.w3schools.com/python/)

### Links para praticar

[https://coddy.tech/journeys/python/](https://coddy.tech/journeys/python/)

[https://exercism.org/tracks/python](https://exercism.org/tracks/python)

[https://www.hackerrank.com/domains/python](https://www.hackerrank.com/domains/python)

---

## 📌 1. Fundamentos da Computação & Internet

- [ ]  [[Como a internet funciona]]
- [ ]  [[HTTP vs HTTPS]]
- [ ]  [[Como funciona um DNS]]
- [ ]  [[Domínio - O que é]]
- [ ]  [[Browsers]] (request lifecycle)
- [ ]  [[Arquitetura Cliente X Servidor]]
- [ ]  [[CLI e Terminal]]
- [ ]  [[Ambientes Virtuais]]

---

## 📌 2. Base de Programação — Python Core

- [ ]  [[Sintaxe básica Python]]
- [ ]  [[Indentação]]
- [ ]  [[Tipos de dados]]
- [ ]  [[Variáveis e escopo]]
- [ ]  [[Condicionais e loops]]
- [ ]  [[List]]
- [ ]  [[Tuple]]
- [ ]  [[Set]]
- [ ]  [[Dict]]
- [ ]  [[Strings]]
- [ ]  [[Funções]]
- [ ]  [[Módulos em Python]]
- [ ]  [[Pacotes]]
- [ ]  [[Importações]]
- [ ]  [[Manipulação de arquivos]]
- [ ]  [[Estruturas de dados]]
- [ ]  [[Comprehensions]]
- [ ]  [[Generators]]
- [ ]  [[Iterators]]

---

## 📌 3. Programação Orientada a Objetos — OOP

- [ ]  [[Classes e Objetos]]
- [ ]  [[Encapsulamento]]
- [ ]  [[Herança]]
- [ ]  [[Polimorfismo]]
- [ ]  [[Abstração]]
- [ ]  [[Interfaces em Python]]
- [ ]  [[Protocolos]]
- [ ]  [[Dunder Methods]]
- [ ]  [[Properties]]
- [ ]  [[Dataclasses]]
- [ ]  [[Imutabilidade]]

---

## 📌 4. Python Moderno

- [ ]  [[Type Hints]]
- [ ]  [[PEP 8]]
- [ ]  [[Decorators]]
- [ ]  [[Context Managers]]
- [ ]  [[Lambdas]]
- [ ]  [[Map Filter Reduce]]
- [ ]  [[Annotations]]
- [ ]  [[Optional em Python]]
- [ ]  [[Pydantic]]
- [ ]  [[Ruff]]
- [ ]  [[Black]]
- [ ]  [[Mypy]]
- [ ]  [[Poetry]]
- [ ]  [[uv]]

---

## 📌 5. Concorrência & Performance

- [ ]  [[Threads em Python]]
- [ ]  [[Multiprocessing]]
- [ ]  [[GIL]]
- [ ]  [[Async Await]]
- [ ]  [[Asyncio]]
- [ ]  [[Event Loop]]
- [ ]  [[Concorrência vs Paralelismo]]
- [ ]  [[Código bloqueante vs não bloqueante]]
- [ ]  [[Profiling de código Python]]

---

## 📌 6. Banco de Dados

### Fundamentos

- [ ]  [[SQL e Bancos de Dados Relacionais]]
- [ ]  [[ACID]]
- [ ]  [[Transações]]
- [ ]  [[Normalização vs Desnormalização]]

### Python + Banco

- [ ]  [[SQLAlchemy]]
- [ ]  [[SQLModel]]
- [ ]  [[Alembic]]
- [ ]  [[psycopg]]
- [ ]  [[asyncpg]]
- [ ]  [[Connection Pooling]]
- [ ]  [[Repository Pattern]]

### Performance

- [ ]  [[Indexação eficiente]]
- [ ]  [[Evitar SELECT *]]
- [ ]  [[Paginação eficiente]]
- [ ]  [[N+1 problem]]
- [ ]  [[Query tuning]]
- [ ]  [[Lazy vs Eager loading]]

### Escala

- [ ]  [[Replicação]]
- [ ]  [[Sharding]]
- [ ]  [[Teorema CAP]]

### Tipos

- [ ]  [[PostgreSQL]]
- [ ]  [[MySQL]]
- [ ]  [[MongoDB]]
- [ ]  [[Redis]]
- [ ]  [[Vector Database]]

---

## 📌 7. Backend & APIs

- [ ]  [[REST]]
- [ ]  [[RESTful]]
- [ ]  [[JSON APIs]]
- [ ]  [[OpenAPI]]
- [ ]  [[Swagger]]
- [ ]  [[GraphQL]]
- [ ]  [[gRPC]]
- [ ]  [[Webhooks]]
- [ ]  [[Versionamento de APIs]]

---

## 📌 8. Framework Python — CORE DO MERCADO

- [ ]  [[FastAPI Roadmap (2026)]] — FOCO TOTAL
- [ ]  [[Django REST Framework]]
- [ ]  [[Flask]]

### FastAPI Core

- [ ]  [[Rotas no FastAPI]]
- [ ]  [[Schemas com Pydantic]]
- [ ]  [[Dependency Injection no FastAPI]]
- [ ]  [[Middlewares]]
- [ ]  [[Background Tasks]]
- [ ]  [[Exception Handlers]]
- [ ]  [[Upload e Download de Arquivos]]
- [ ]  [[Autenticação no FastAPI]]

---

## 📌 9. Segurança

- [ ]  [[JWT]]
- [ ]  [[OAuth2]]
- [ ]  [[OpenID Connect]]
- [ ]  [[Authentication vs Authorization]]
- [ ]  [[Hashing]] — bcrypt, Argon2
- [ ]  [[HTTPS]]
- [ ]  [[SSL]]
- [ ]  [[OWASP Top 10]]
- [ ]  [[Rate Limiting]]
- [ ]  [[Throttling]]
- [ ]  [[CORS]]
- [ ]  [[Secrets Management]]

---

## 📌 10. Caching

- [ ]  [[Cache]]
- [ ]  [[HTTP Cache]]
- [ ]  [[Server Cache]]
- [ ]  [[CDN]]
- [ ]  [[Redis]]
- [ ]  [[Memcached]]

### Cache Patterns

- [ ]  [[Cache Aside]]
- [ ]  [[Write Through]]
- [ ]  [[Read Through]]
- [ ]  [[Write Behind]]
- [ ]  [[Cache Invalidation]]

---

## 📌 11. Arquitetura de Software

- [ ]  [[Clean Architecture]]
- [ ]  [[Hexagonal Architecture]]
- [ ]  [[DDD]]
- [ ]  [[SOLID]]
- [ ]  [[Design Patterns]]

### Arquiteturas

- [ ]  [[Monolito]]
- [ ]  [[Microsserviços]]
- [ ]  [[SOA]]
- [ ]  [[Serverless]]

### Padrões avançados

- [ ]  [[CQRS]]
- [ ]  [[Event Sourcing]]
- [ ]  [[Unit of Work]]
- [ ]  [[Repository Pattern]]
- [ ]  [[Service Layer]]

---

## 📌 12. Comunicação & Assincronismo

- [ ]  [[RabbitMQ]]
- [ ]  [[Kafka]]
- [ ]  [[Mensageria]]
- [ ]  [[Background Jobs]]
- [ ]  [[Event-driven architecture]]
- [ ]  [[Celery]]
- [ ]  [[RQ]]
- [ ]  [[Dramatiq]]
- [ ]  [[Workers]]

---

## 📌 13. Escalabilidade & Resiliência

- [ ]  [[Load Balancing]]
- [ ]  [[Horizontal vs Vertical Scaling]]
- [ ]  [[Circuit Breaker]]
- [ ]  [[Retry Pattern]]
- [ ]  [[Backpressure]]
- [ ]  [[Graceful Degradation]]
- [ ]  [[Graceful Shutdown]]
- [ ]  [[Health Checks]]
- [ ]  [[Timeouts]]

---

## 📌 14. Performance Backend

- [ ]  [[Compressão de respostas]]
- [ ]  [[Redução de payload]]
- [ ]  [[Streaming de dados]]
- [ ]  [[Batch requests]]
- [ ]  [[Minimizar latência]]
- [ ]  [[Profiling de código]]
- [ ]  [[Otimização de algoritmos]]
- [ ]  [[Uvicorn]]
- [ ]  [[Gunicorn]]
- [ ]  [[Workers]]
- [ ]  [[Async HTTP]]
- [ ]  [[Locust]]

---

## 📌 15. Observabilidade

- [ ]  [[Logs]]
- [ ]  [[Structured Logging]]
- [ ]  [[Correlation ID]]
- [ ]  [[Metrics]]
- [ ]  [[Tracing]]

Ferramentas:

- [ ]  [[Prometheus]]
- [ ]  [[Grafana]]
- [ ]  [[ELK Stack]]
- [ ]  [[OpenTelemetry]]
- [ ]  [[Jaeger]]
- [ ]  [[Sentry]]

---

## 📌 16. Testes

- [ ]  [[Testes Unitários]]
- [ ]  [[Testes Integrados]]
- [ ]  [[Testes de API]]
- [ ]  [[Pytest]]
- [ ]  [[Fixtures]]
- [ ]  [[Mocking]]
- [ ]  [[Coverage]]
- [ ]  [[Factory Boy]]
- [ ]  [[Faker]]
- [ ]  [[Testcontainers]]
- [ ]  [[Teste de carga]]
- [ ]  [[Locust]]

---

## 📌 17. DevOps & Infra

- [ ]  [[Git]]
- [ ]  [[CICD]]
- [ ]  [[O mapa do arquiteto/01 - Do zero ao Jr/07 Docker/Docker]]
- [ ]  [[Docker Compose]]
- [ ]  [[Kubernetes]]
- [ ]  [[Nginx]]
- [ ]  [[Apache]]
- [ ]  [[Linux para Backend]]
- [ ]  [[Variáveis de Ambiente]]
- [ ]  [[Secrets]]

---

## 📌 18. Gerenciamento de Dependências & Build

- [ ]  [[pip]]
- [ ]  [[venv]]
- [ ]  [[Poetry]]
- [ ]  [[uv]]
- [ ]  [[requirements.txt]]
- [ ]  [[pyproject.toml]]
- [ ]  [[Dockerfile Python]]
- [ ]  [[Multi-stage Build]]

---

## 📌 19. Real-time & Avançado

- [ ]  [[WebSockets]]
- [ ]  [[SSE]]
- [ ]  [[Long Polling]]
- [ ]  [[FastAPI WebSockets]]
- [ ]  [[Streaming APIs]]
- [ ]  [[Notificações em tempo real]]

---

## 📌 20. System Design

- [ ]  [[Designing for scale]]
- [ ]  [[Trade-offs]]
- [ ]  [[Latência vs Consistência]]
- [ ]  [[Sistemas distribuídos]]
- [ ]  [[Idempotência]]
- [ ]  [[Consistência eventual]]
- [ ]  [[CAP na prática]]
- [ ]  [[Arquitetura Cloud-native]]

---

## 📌 21. Diferenciais Python 2026+

### IA Aplicada

- [ ]  [[Integração com APIs de IA]]
- [ ]  [[RAG]]
- [ ]  [[Embeddings]]
- [ ]  [[Vector Databases]]
- [ ]  [[LangChain]]
- [ ]  [[LlamaIndex]]
- [ ]  [[Agentes de IA]]

### Dados

- [ ]  [[Pandas]]
- [ ]  [[Polars]]
- [ ]  [[PySpark]]
- [ ]  [[Airflow]]
- [ ]  [[dbt]]
- [ ]  [[ETL]]
- [ ]  [[ELT]]
- [ ]  [[Data Pipelines]]

---

# 🧭 Ordem Recomendada

1. Fundamentos da Computação & Internet
2. Python Core
3. OOP em Python
4. Python Moderno
5. FastAPI
6. Banco de Dados com SQLAlchemy/Alembic
7. Segurança
8. Testes com Pytest
9. Docker
10. Arquitetura Limpa
11. Async / Await
12. Redis e Caching
13. Mensageria
14. Observabilidade
15. Cloud / DevOps
16. System Design
17. IA aplicada e Dados

---

# 🎯 Foco Estratégico

Esse roadmap posiciona Python como linguagem para:

- APIs leves e rápidas
- automações internas
- integrações com IA
- pipelines de dados
- microsserviços 
- BFFs
- ferramentas internas
- protótipos de produto